...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagAC_OnOff)
    {
        info->event[info->num] = EVENT_AC_OnOff;
        info->num++;
        dataPoints->valueAC_OnOff = gizStandardDecompressionValue(AC_OnOff_BYTEOFFSET,AC_OnOff_BITOFFSET,AC_OnOff_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueAC_OnOff != cur->valueAC_OnOff)
    {
        GIZWITS_LOG("valueAC_OnOff Changed\n");
        ret = 1;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(AC_OnOff_BYTEOFFSET,AC_OnOff_BITOFFSET,AC_OnOff_LEN,(uint8_t *)devStatusPtr,dataPoints->valueAC_OnOff);
